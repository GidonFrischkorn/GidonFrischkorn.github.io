...
#let parse_icon_string(icon_string) = {
  if icon_string.starts-with("fa ") [
    ...
    #if parts.len() == 2 {
      fa-icon(parts.at(1), font: ("Font Awesome 5 Free Solid", "Font Awesome 5 Free"), fill: color-darknight)
    } else if parts.len() == 3 and parts.at(1) == "brands" {
      fa-icon(parts.at(2), font: "Font Awesome 5 Brands", fill: color-darknight)
    } else {
...
#let resume(
  ...
  show heading.where(level: 1): it => [
    ...
    #context {
      // Extract plain text from the heading body, handling both
      // simple text and Pandoc-generated content sequences
      // (e.g., headings containing & produce a sequence in Typst)
      let heading-text = if type(it.body) == content and it.body.has("text") {
        it.body.text
      } else if type(it.body) == content and it.body.has("children") {
        it.body.children.map(c => {
          if type(c) == content and c.has("text") { c.text }
          else if c == [ ] { " " }
          else { "" }
        }).join()
      } else {
        repr(it.body).replace("\"", "").replace("[", "").replace("]", "")
      }
      let first-three = if heading-text.len() >= 3 { heading-text.slice(0, 3) } else { heading-text }
      let rest = if heading-text.len() >= 3 { heading-text.slice(3) } else { "" }
      align(left)[
        #text[#strong[#text(state-color-accent.get())[#first-three]#text(
            color-darkgray,
          )[#rest]]]
